...
import { FiTool, FiAlertOctagon, FiAlertTriangle, FiPhoneCall, FiSettings } from 'react-icons/fi'
import { AdminContext } from '../context/AdminContext'

const MaintenanceMode = ({ isKillSwitch, message }) => {
    const { adminProfile, setatoken } = useContext(AdminContext)

    // Master admins are NEVER blocked by this overlay 
    // (They are the ones who control the switch!)
            ...
            <div className="relative max-w-lg w-full bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[32px] p-8 md:p-12 text-center shadow-2xl overflow-hidden">
                ...
                <div className="flex flex-col items-center">
                    ...
                    <h1 className="text-3xl md:text-4xl font-black text-white mb-4 tracking-tighter uppercase">
                        {isKillSwitch ? "Global Suspension" : "Admin Maintenance"}
                    </h1>

                    <div className="w-12 h-1 bg-white/20 rounded-full mb-6" />

                    <p className="text-slate-300 text-lg leading-relaxed font-medium mb-8">
                        {message || (isKillSwitch ? "Critical emergency shutdown active. All administrative and medical services are temporarily suspended." : "The management portal is currently limited for scheduled maintenance. Please wait for completion.")}
                    </p>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 w-full">
                        <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-4 transition-colors hover:bg-white/10">
                            ...
                            <p className="text-sm font-bold text-white tracking-tight">{isKillSwitch ? "Emergency Lockdown" : "Portal Maintenance"}</p>
                        </div>
                        <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-4 transition-colors hover:bg-white/10">
                            <FiSettings className="text-indigo-400 mx-auto mb-2" size={24} />
                            <p className="text-[10px] uppercase font-black text-slate-400 tracking-widest mb-1">Control</p>
                            <p className="text-[11px] font-bold text-white/60 leading-tight">Master Admin Required to bypass</p>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className="mt-8 text-xs font-bold text-slate-500 hover:text-white transition-colors underline decoration-dotted underline-offset-4"
                    >
                        Logout and Exit
                    </button>
                </div>
                ...
                <div className="mt-12 flex items-center justify-center gap-3 opacity-40">
                    <p className="text-[10px] font-bold text-white uppercase tracking-[0.2em]">PawVaidya Admin OS</p>
                </div>
            </div>
