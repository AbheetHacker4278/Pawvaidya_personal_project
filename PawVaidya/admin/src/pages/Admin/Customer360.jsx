...
import {
  FaSearch, FaWallet, FaPaw, FaCalendarAlt, FaExclamationTriangle, FaUser,
  FaChartPie, FaCrown, FaHistory, FaUndo, FaTimes, FaStethoscope,
  FaClipboardList, FaHeartbeat, FaMedkit, FaAppleAlt, FaLeaf, FaFlask,
  FaTint, FaHeart, FaCoins, FaClock, FaMapMarkerAlt, FaBan, FaCheckCircle,
  FaInfoCircle, FaGoogle
} from 'react-icons/fa';
...
const Customer360 = () => {
  ...
  const handleTriggerEmergency = async (alert) => {
    try {
      const { data } = await axios.post(`${backendurl}/api/admin/trigger-emergency`, {
      ...
      for (const pred of customerData.mlPredictions) {
        ...
        if (isAbnormal) {
          ...
          if (evalResult.vitalsEvaluation.heartRate === 'Abnormal') {
            abnormalDetails.push(`heart/pulse rate of ${pred.vitals?.pulseRate || pred.vitals?.heartRate} bpm`);
          }
          if (evalResult.vitalsEvaluation.respiratoryRate === 'Abnormal') {
            abnormalDetails.push(`respiratory rate of ${pred.vitals?.respirationRate || pred.vitals?.respiratoryRate} /m`);
          }
  ...
  const fetchCustomerData = async (e) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.get(`${backendurl}/api/admin/user-360/${email}`, {
        headers: { atoken }
      });
      if (data.success) {
  ...
  const handleRefund = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${backendurl}/api/admin/refund`, {
      ...
      if (data.success) {
        ...
        fetchCustomerData(); // Refresh to show refund status
      } else {
        toast.error(data.message);
      }
    } catch (error) {
  ...
  const handleReclaimRefund = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${backendurl}/api/admin/reclaim-refund`, {
        email: customerData.user.email,
  ...
  const handleGrantSubscription = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${backendurl}/api/admin/grant-subscription`, {
        userId: customerData.user._id,
        plan: grantPlan,
  ...
  const handleRevokeSubscription = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${backendurl}/api/admin/revoke-subscription`, {
        userId: customerData.user._id,
        reason: revokeReason,
      ...
      if (data.success) {
        toast.success(data.message || 'Subscription revoked successfully.');
        setRevokeModalOpen(false);
        setRevokeReason('');
        setShouldRefund(false);
        fetchCustomerData();
      } else {
        toast.error(data.message || 'Failed to revoke subscription.');
      }
    } catch (error) {
    ...
    <div className="p-6 space-y-6">
      ...
      {customerData && customerData.user ? (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-gradient-to-r from-slate-800 to-slate-900 px-6 py-8 relative">
              <div className="flex flex-col md:flex-row items-center gap-6">
                ...
                <div className="text-white space-y-2 text-center md:text-left">
                  ...
                  <p className="text-slate-300 flex flex-wrap justify-center md:justify-start items-center gap-2">
                ...
                {/* Wallet Info Box */}
                <div className="md:ml-auto bg-white/10 backdrop-blur-md border border-white/20 p-5 rounded-2xl text-white text-center min-w-[240px] w-full md:w-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {customerData.pets && customerData.pets.length > 0 ? customerData.pets.map(pet => (
            ...
            <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {customerData.appointments && customerData.appointments.length > 0 ? customerData.appointments.map(app => (
            ...
            <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {customerData.crueltyReports && customerData.crueltyReports.length > 0 ? customerData.crueltyReports.map(rep => (
          ...
          {(() => {
            const obsidianSub = customerData.subscriptions?.find(s => s.plan === 'Obsidian' && ['Pending', 'Approved', 'Active'].includes(s.status)) ||
                                (customerData.user.subscription?.plan === 'Obsidian' ? customerData.user.subscription : null);
            if (!obsidianSub) return null;
            
            const isApproved = obsidianSub.status === 'Approved';
            const isPending = obsidianSub.status === 'Pending' || obsidianSub.status === 'Pending Approval';
            const isActive = obsidianSub.status === 'Active';
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                  <div className="bg-white/5 border border-white/10 p-3.5 rounded-xl">
                    ...
                    <span className={`text-sm font-black uppercase tracking-wider ${
                      ...
                      isApproved ? 'text-indigo-400 animate-pulse' :
                      isPending ? 'text-amber-400' :
                      'text-rose-400'
                    }`}>
                      {isActive ? 'Active' : isApproved ? 'Approved' : isPending ? 'Pending Approval' : obsidianSub.status}
                    </span>
                  </div>
                  <div className="bg-white/5 border border-white/10 p-3.5 rounded-xl">
                    <span className="text-xs text-slate-400 block mb-0.5 font-bold">Expiry / Grace Period End</span>
                    <span className="text-sm font-bold text-slate-200">
                      {isApproved && obsidianSub.approvedAt ? (
                        (() => {
                          const limit = new Date(obsidianSub.approvedAt).getTime() + 24 * 60 * 60 * 1000;
                        ...
                        obsidianSub.expiryDate ? new Date(obsidianSub.expiryDate).toLocaleDateString() : 'N/A'
                      )}
                    </span>
                  </div>
                  <div className="bg-white/5 border border-white/10 p-3.5 rounded-xl">
                    ...
                    <span className="text-sm font-bold text-slate-200">
                      {obsidianSub.amount ? `₹${obsidianSub.amount.toLocaleString()}` : 'Custom Premium'}
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  <div key={sub._id} className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase ${sub.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : sub.status === 'Pending' ? 'bg-amber-100 text-amber-700 border border-amber-200 animate-pulse' : 'bg-slate-200 text-slate-600'}`}>
                          {sub.status}
                        </span>
                      </div>
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase ${customerData.user.subscription.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : customerData.user.subscription.status === 'Pending' ? 'bg-amber-100 text-amber-700 border border-amber-200 animate-pulse' : 'bg-slate-200 text-slate-600'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mt-6">
            <div className="flex items-center justify-between mb-5 pb-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Admin-issued retention coupons active for this subscriber</p>
              </div>
            ...
            {customerData.activeLoyaltyCoupons && customerData.activeLoyaltyCoupons.length > 0 ? (
                ...
                {customerData.activeLoyaltyCoupons.map((coupon) => {
                  ...
                  return (
                    <div key={coupon.code} className={`relative overflow-hidden rounded-2xl border-2 p-5 ${
                      ...
                      {isWellness && (
                        <div className="mt-2 px-3 py-1.5 bg-emerald-100/60 rounded-lg">
                          <p className="text-[10px] font-bold text-emerald-700">🤖 AI Triggered: High churn risk detected — free wellness checkup issued to retain subscriber</p>
                        </div>
                      )}
                      {!isWellness && (
                        <div className="mt-2 px-3 py-1.5 bg-violet-100/60 rounded-lg">
                          <p className="text-[10px] font-bold text-violet-700">🤖 AI Triggered: Medium churn risk — 15% loyalty discount issued to re-engage subscriber</p>
                        </div>
                      )}
                    </div>
                  );
              ...
              <div className="flex flex-col items-center justify-center py-10 text-slate-400">
                ...
                <p className="text-xs font-medium text-slate-400 mt-1">This user has no active loyalty or wellness coupons at this time.</p>
                <p className="text-xs text-slate-300 mt-0.5">Rewards are issued from the All-Subscriptions → AI Prediction → Re-engage workflow.</p>
              </div>
            )}
          </div>
        ...
        !loading && (
          <div className="text-center py-24 bg-white rounded-2xl border border-slate-100 shadow-sm">
            <FaUser className="text-slate-300 text-6xl mx-auto mb-4" />
            <h3 className="text-xl font-bold text-slate-700">No Profile Loaded</h3>
            <p className="text-sm text-slate-400 mt-2 max-w-md mx-auto">
              Please enter an email address in the search box above to access unified telemetry, emergency overrides, and subscription controls.
            </p>
          </div>
        )
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleRevokeSubscription} className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2 py-2">
                ...
                />
                <label htmlFor="shouldRefund" className="text-sm font-bold text-slate-700 select-none">
                  Refund paid subscription amount to PawWallet
                </label>
              </div>
              <div className="flex gap-4 pt-2">
