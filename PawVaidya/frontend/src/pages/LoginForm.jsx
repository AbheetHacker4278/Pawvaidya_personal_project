...
import { Mail, Lock, Shield, Heart, Sparkles, Activity, Plus } from 'lucide-react';
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-6 sm:p-12 bg-transparent z-10">
        ...
        >
          ...
          <div 
            ...
            className="space-y-2 mb-8 bg-transparent"
          >
            <h1 className="text-4xl font-extrabold bg-gradient-to-r from-[#2c5b40] via-[#489065] to-[#2e5b40] bg-clip-text text-transparent animate-gradient">
              Welcome Back!
            </h1>
            <p className="text-gray-600 font-medium">Access expert advice for your furry friends</p>
          </div>

          {/* Form with 3D Elements */}
          <form onSubmit={onSubmitHandler} className="space-y-5 bg-transparent">
            
            {/* Email Field with translateZ */}
            ...
            {/* Password Field with translateZ */}
            <div style={{ transform: "translateZ(25px)" }}>
              <FormInput
                type="password"
                name="password"
            ...
            >
              <div className="flex items-center gap-2 bg-transparent group cursor-pointer">
                <input
                  type="checkbox"
                  name="remember"
                  id="remember"
                  ...
                  onChange={handleInputChange}
                  className="w-4 h-4 rounded border-gray-300 text-[#489065] focus:ring-[#489065]/30 cursor-pointer transition-transform duration-200 hover:scale-110"
                />
                <label htmlFor="remember" className="text-sm font-semibold text-gray-600 bg-transparent cursor-pointer group-hover:text-gray-800 transition-colors duration-200">
                  Remember me
                </label>
              </div>
              <p
                onClick={() => navigate('/reset-password')}
                className="text-sm font-semibold bg-transparent text-[#5c649c] hover:text-[#424975] cursor-pointer transition-all duration-200 hover:scale-105"
              >
                Forgot password?
              </p>
            </div>

            {/* Submit Button with translateZ & hover shimmers */}
            ...
            <div style={{ transform: "translateZ(20px)" }}>
              <motion.button
                whileHover={{ scale: 1.025, translateZ: "25px" }}
                whileTap={{ scale: 0.98 }}
                type="button"
                onClick={() => navigate('/login')}
                className="w-full bg-gradient-to-r from-[#656ca3] to-[#393d60] text-white py-4 rounded-xl font-bold hover:from-[#393d60] hover:to-[#656ca3] transition-all duration-300 shadow-md hover:shadow-lg"
              >
                <span>Sign Up</span>
                <span className='text-xl bg-transparent ml-2'>🦥</span>
              </motion.button>
            ...
            {/* Biometric Login */}
            <div style={{ transform: "translateZ(25px)", transformStyle: "preserve-3d" }} className="group/biometric">
              <motion.button
                whileHover={{ scale: 1.025, translateZ: "30px" }}
                whileTap={{ scale: 0.98 }}
                type="button"
                onClick={() => setShowFaceAuth(true)}
                className="w-full mt-2 relative overflow-hidden bg-white/30 hover:bg-white/50 text-[#2e5b40] border border-[#489065]/40 py-4 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2 shadow-sm hover:shadow-md"
              >
                <Shield size={20} className="text-[#489065] animate-pulse" />
                <span>Biometric Login</span>
                {/* Laser scan line effect */}
                <div className="absolute left-0 right-0 h-[2px] bg-emerald-500 shadow-[0_0_8px_#10b981] opacity-0 group-hover/biometric:opacity-100 animate-laser-scan pointer-events-none" />
              </motion.button>
            </div>
          </form>
